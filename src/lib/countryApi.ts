...
async function allCountriesRequest(): Promise<AllCountriesResponse | undefined> {
  try {
    const response = await axios.get<Country[]>(`https://restcountries.com/v3.1/all?fields=name,independent,cca3,capital,landlocked,maps,population,flags,region,subregion`);
...
export function selectCountry(countriesArray: string[], countriesResponse: Country[], countryCodeMapping: CountryCodeMapping[]): CountryObj {
